if (typeof window === 'undefined') {
  globalThis.window = {addEventListener: () => {}, clinicalCharts: {}, innerWidth: 1024, scrollY: 0, scrollX: 0};
}
...
export function getCanonicalCode(code) {
  ...
  const map = {EGFR: 'GFR_CKD_EPI', GFR: 'GFR_CKD_EPI', 'NEUT#': 'NEU#', 'NEUT%': 'NEU%', 'LYMPH#': 'LYM#', 'LYMPH%': 'LYM%', SODYUM: 'NA', POTASYUM: 'K', KALSIYUM: 'CA', KALSİYUM: 'CA'};
...
export default function registerDashboard(Alpine) {
  Alpine.data('dashboardApp', () => ({
    ...
    tests: [],
    testItems: [],
    metrics: {avgSys: 0, avgDia: 0, currentWeight: 0, startWeight: 0, weightDelta: 0, outOfBoundsCount: 0, outOfBoundsList: [], latestInr: null, latestHgb: null, bpStatusText: '', bpStatusClass: ''},
    flowsheetDays: [],
    allFlowsheetMeds: [],
    onlyMeds: false,
    uniqueMeds: [],
    ...
    hasRecordsInRange(medName) {
      ...
      const hasLog = this.medicationLogs.some((l) => l.med.toLowerCase() === medName.toLowerCase() && l.at.substring(0, 10) >= start && l.at.substring(0, 10) <= end);
      if (hasLog) return true;
      const hasChange = this.medicationChanges.some((c) => {
        const med = this.medications.find((m) => m.id === c.medication_id);
        if (!med) return false;
        return med.name.toLowerCase() === medName.toLowerCase() && c.at.substring(0, 10) >= start && c.at.substring(0, 10) <= end;
      });
      return hasChange;
    },
    ...
    _triggerLoader(sectionKey) {
      const map = {summary: 'loadMetrics', prescriptions: 'loadTimeline', charts: 'loadBpChart', labTrends: 'loadLabCharts', flowsheet: 'loadFlowsheet'};
      const method = map[sectionKey];
    ...
    calculateMetrics() {
      ...
      if (testsBeforeEnd.length > 0) {
        ...
        itemsOfLatestTest.forEach((item) => {
          ...
          if (status !== 'Normal') {
            outList.push({code: getCanonicalCode(item.code), result: item.result, date: latestTest.at.substring(5, 10), rawDate: latestTest.at, status: status});
          }
        });
      }
    ...
    get visibleMeds() {
      ...
      return this.detailedMeds
        ...
        .filter((m) => !this.onlyActive || this.hasRecordsInRange(m.name))
        .map((med) => {
    ...
    get clinicalNarrative() {
      ...
      return {surgeryType: ctx.surgery_type || '', valveType: ctx.valve_type || '', targetBpMax: ctx.target_bp_sys_max || null, targetBpMin: ctx.target_bp_sys_min || null};
    },
    ...
    _renderMainChartsInternal() {
      ...
      if (bpCtx && fps.length > 0) {
        ...
        window.clinicalCharts['bp'] = new Chart(bpCtx, {
          ...
          data: {
            datasets: [
              {label: 'Sistolik', data: sysData, borderColor: 'rgb(225, 29, 72)', backgroundColor: 'rgba(225, 29, 72, 0.03)', pointRadius: 1, borderWidth: 2, tension: 0.2, fill: true, spanGaps: true},
              {label: 'Diastolik', data: diaData, borderColor: 'rgb(79, 70, 229)', backgroundColor: 'rgba(79, 70, 229, 0.03)', pointRadius: 1, borderWidth: 2, tension: 0.2, fill: true, spanGaps: true},
            ],
          },
          options: {
            responsive: true,
      ...
      if (wCtx && fws.length > 0) {
        ...
        window.clinicalCharts['weight'] = new Chart(wCtx, {
          type: 'line',
          data: {datasets: [{label: 'Kilo (kg)', data: weightData, borderColor: 'rgb(13, 148, 136)', backgroundColor: 'rgba(13, 148, 136, 0.03)', pointRadius: 1, borderWidth: 2, tension: 0.2, fill: true}]},
          options: {
            responsive: true,
            maintainAspectRatio: false,
            animation: false,
    ...
    _renderFocusChartInternal() {
      ...
      medsToShow.forEach((medName) => {
        ...
        datasets.push({label: medName, data: data, backgroundColor: cfg.color, borderColor: cfg.color, borderRadius: 4, borderWidth: 1});
      });
    ...
    get labTrendTargets() {
      ...
      codes.forEach((code) => {
        ...
        if (sample) {
          result.push({id: 'chart_' + code.toLowerCase().replace(/[^a-z0-9]/g, (c) => '_' + c.charCodeAt(0).toString(16)), code: code, label: sample.name || code, color: this.getColorForCode(code)});
        }
      });
      return result;
    },
      ...
      targets.forEach((tgt) => {
        ...
        this.tests.forEach((s) => {
          ...
          if (match) {
            ...
            allPoints.push({val: parseFloat(match.result), date: s.at.substring(0, 10), rawDate: s.at, refMin: effective.min, refMax: effective.max});
          }
        });
        allPoints.sort((a, b) => new Date(a.rawDate) - new Date(b.rawDate));
        let insideRange = allPoints.filter((p) => p.date >= this.startDate && p.date <= this.endDate);
        ...
        window.clinicalCharts[tgt.id] = new Chart(ctx, {
          ...
          data: {
            datasets: [
              {label: 'Max Sınır', data: maxData, borderColor: 'rgba(34, 197, 94, 0.3)', borderWidth: 1, borderDash: [2, 2], pointRadius: 0, tension: 0, fill: false, spanGaps: true},
              {label: 'Min Sınır', data: minData, borderColor: 'rgba(34, 197, 94, 0.3)', borderWidth: 1, borderDash: [2, 2], pointRadius: 0, tension: 0, fill: 0, backgroundColor: 'rgba(34, 197, 94, 0.1)', spanGaps: true},
              {label: tgt.label, data: dData, borderColor: tgt.color, backgroundColor: tgt.color.replace('rgb', 'rgba').replace(')', ',0.1)'), borderWidth: 1.5, pointRadius: 2.5, tension: 0.15, fill: false, spanGaps: true},
            ],
          },
          options: {
            responsive: true,
    ...
    getEventBgClass(e) {
      ...
      const map = {med: 'bg-indigo-50/20 border-indigo-100/40 hover:bg-indigo-50/40', weight: 'bg-sky-50/70 border-sky-200 hover:bg-sky-100', test: 'cursor-pointer bg-teal-100 border-teal-300 hover:bg-teal-200 shadow-sm', report: 'cursor-pointer bg-violet-100 border-violet-300 hover:bg-violet-200 shadow-sm', medchange: 'bg-amber-50/10 border-amber-100/30 hover:bg-amber-50/30'};
    ...
    getDailyEvents(dateStr) {
      ...
      this.medicationChanges.forEach((c) => {
        if (c.at.substring(0, 10) === dateStr) {
          ...
          if (c.type !== 'Taken') {
            events.push({type: 'medchange', time: c.at.substring(11, 16), med: medName, changeType: c.type, amount: c.amount, timespan: c.timespan, description: description, isPlan: false});
          }
        }
      });
    ...
    getDailyEventsSummary(dateStr, onlyMeds) {
      ...
      const labels = {bp: 'Tansiyon', med: 'İlaç', weight: 'Kilo', test: 'Tahlil', report: 'Rapor', medchange: 'İlaç Değişimi'};
      const result = [];
    ...
    async downloadRawMedicalData(mode = 'all') {
      try {
        ...
        const clinicalContextSummary = {patient_summary: ctx.patient_summary || '', anatomical_surgical_modifications: ctx.anatomical_surgical_modifications || [], emergency_referral_pathway: ctx.emergency_referral_pathway || [], clinical_timeline: ctx.clinical_timeline || []};
        ...
        if (mode === 'period') {
          ...
          clinicalInsightsData = {selected_period: {label: 'Seçili Dönem', range: {start: this.startDate, end: this.endDate}, insights: [{period: 'all', range: {start: this.startDate, end: this.endDate}, html: this.generateDynamicInsight(this.startDate, this.endDate)}]}};
        } else {
          const presetToPeriod = {3: 'last3', 7: 'last7', 10: 'last10', 15: 'last15', 30: 'last30', 45: 'last45', 60: 'last60', 90: 'last90'};
          ...
          clinicalInsightsData = {
            selected_period: {label: 'Seçili Dönem', range: {start: this.startDate, end: this.endDate}, insights: selectedPeriods.map((p) => ({period: p.label, range: {start: p.start, end: p.end}, html: this.generateDynamicInsight(p.start, p.end)}))},
            all_time: {label: 'Tüm Zamanlar', range: {start: this.firstD, end: this.lastD}, insights: allTimePeriods.map((p) => ({period: p.label, range: {start: p.start, end: p.end}, html: this.generateDynamicInsight(p.start, p.end)}))},
          };
        }
        const masterOutput = {
